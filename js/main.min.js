...
try { document.execCommand('copy'); showToast('Скопировано! Вставьте в чат Telegram (Ctrl+V)'); }
catch(e) { showToast('Текст заказа не скопирован — вставьте вручную'); }
...
var bn = document.getElementById('bottomNav');
if (bn) bn.classList.remove('hidden');
var _reviewsUnlockTimer = setTimeout(function() {
modal.removeEventListener('transitionend', handler);
unlockBody();
}, 400);
function handler() {
clearTimeout(_reviewsUnlockTimer);
modal.removeEventListener('transitionend', handler);
unlockBody();
}
...
const LAYOUTS = [
{ side:'left', tp: 8, rot: -14 }, 
{ side:'right', tp: 27, rot: 18 }, 
{ side:'left', tp: 27, rot: -18 }, 
{ side:'right', tp: 8, rot: 14 }, 
{ side:'left', tp: 50, rot: 8 }, 
{ side:'right', tp: 50, rot: -8 }, 
{ side:'left', tp: 71, rot: -11 }, 
{ side:'right', tp: 71, rot: 11 }, 
];
const FLOATS = [
...
if(typeGen !== capturedWaitGen) { waitTimer = null; return; } 
waitTimer = null;
hideArrows();
STATE = 'zoom_out';
}, WAIT_DURATION);
...
let _cachedLayout = null, _cachedLayoutW = 0;
...
const secEl = cachedSecEl || document.getElementById('reviews');
if (!secEl) { if (loopActive) requestAnimationFrame(loop); return; }
if (!cachedSecEl) cachedSecEl = secEl;
...
const minL = MARGIN;
const maxL = secW - THUMB_W - MARGIN;
if (!cachedTrackEl) cachedTrackEl = stageEl.querySelector('.reviews-track') || trackEl;
const trackEl2 = cachedTrackEl;
/* Cache layout values — only recompute when section resizes */
if (!_cachedLayout || _cachedLayoutW !== secW) {
const stgOffLeft = stageEl.offsetLeft;
const stgOffRight = stgOffLeft + stageEl.offsetWidth;
...
_cachedLayout = {
stgOffLeft, stgOffRight,
cardL: stageOff.left, cardW: stageEl.offsetWidth,
cardCenterY: trackOff2.top + trackEl2.offsetHeight / 2
};
_cachedLayoutW = secW;
}
const { stgOffLeft, stgOffRight, cardL, cardW, cardCenterY } = _cachedLayout;
