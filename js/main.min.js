...
function buildMessage() {
  ...
  if (!items) { showToast('Корзина пуста'); return null; }
...
function lockBody() {
  var count = parseInt(document.body.dataset.lockCount || '0', 10);
  if (!Number.isFinite(count) || count < 0) {
    // self-heal: reset any stale lock state
...
function initWaveText(){document.querySelectorAll('.wave-text').forEach(function(el){if(el.querySelector('.w'))return;var frag=document.createDocumentFragment();Array.from(el.childNodes).forEach(function(node){if(node.nodeType!==Node.TEXT_NODE){frag.appendChild(node.cloneNode(true));return;}var parts=(node.textContent||'').split(/(\s+)/);parts.forEach(function(part){if(!part)return;if(/^\s+$/.test(part)){frag.appendChild(document.createTextNode(part));}else{var span=document.createElement('span');span.className='w';span.textContent=part;frag.appendChild(span);}});});el.innerHTML='';el.appendChild(frag);var words=el.querySelectorAll('.w');words.forEach(function(word,i){word.addEventListener('mouseenter',function(){if(words[i-1])words[i-1].classList.add('near');if(words[i+1])words[i+1].classList.add('near');});word.addEventListener('mouseleave',function(){if(words[i-1])words[i-1].classList.remove('near');if(words[i+1])words[i+1].classList.remove('near');});});});}
function initSectionTitleWords(){document.querySelectorAll('.section-title .ht-word,.section-title .ht-em').forEach(function(el){if(el.dataset.splitInit==='1')return;var txt=(el.textContent||'').trim();if(!txt)return;var parts=txt.split(/(\s+)/);el.innerHTML=parts.map(function(p){if(/^\s+$/.test(p))return p;return'<span class="ht-w">'+p+'</span>';}).join('');el.dataset.splitInit='1';});}
function initMessengerRings(){if(!window.matchMedia('(hover: hover) and (pointer: fine)').matches)return;[{btn:'.btn-hero-wa',ring:'#ring-text-wa',flat:'#flat-text-wa'},{btn:'.btn-hero-tg',ring:'#ring-text-tg',flat:'#flat-text-tg'}].forEach(function(cfg){var btn=document.querySelector(cfg.btn),ring=document.querySelector(cfg.ring),flat=document.querySelector(cfg.flat);if(!btn||!ring||!flat)return;var raf=null,state={p:0};function render(){var p=state.p;ring.setAttribute('opacity',String(0.5*(1-p)));ring.setAttribute('transform','translate(0,'+(-18*p).toFixed(2)+')');flat.setAttribute('opacity',String(p));flat.setAttribute('transform','translate(0,'+(-12*p).toFixed(2)+')');flat.setAttribute('font-size',(6.5+(12-6.5)*p).toFixed(2));}function animate(to){if(raf)cancelAnimationFrame(raf);var from=state.p,start=performance.now(),dur=to>from?320:360;function step(ts){var t=Math.min((ts-start)/dur,1),e=1-Math.pow(1-t,3);state.p=from+(to-from)*e;render();if(t<1)raf=requestAnimationFrame(step);}raf=requestAnimationFrame(step);}render();btn.addEventListener('mouseenter',function(){animate(1);});btn.addEventListener('mouseleave',function(){animate(0);});});}
function initApp() {
  renderCatalogNav();
...
function _onScroll() {
  ...
  requestAnimationFrame(() => {
    ...
    const _pct  = total > 0 ? (y / total * 100) : 0;

    if (_scrollEl) _scrollEl.style.width = _pct + '%';
...
// ── PAGEHIDE: clean up all timers to prevent leaks on bfcache ──
window.addEventListener('pagehide', () => {
  // Slider timers
  Object.keys(slideTimers).forEach(k => { clearInterval(slideTimers[k]); delete slideTimers[k]; });
  // New reviews system timers (typewriter + wait)
  if (typeof typeTimer !== 'undefined' && typeTimer) { clearTimeout(typeTimer); }
  if (typeof waitTimer !== 'undefined' && waitTimer) { clearTimeout(waitTimer); }
});
