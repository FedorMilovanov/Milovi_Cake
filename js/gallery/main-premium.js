...
function openLightbox(index){
  ...
  $('#lbRoot').addEventListener('click',e=>{ if(e.target.id==='lbRoot') closeLightbox(); }); $('.lb-backdrop')?.addEventListener('click',e=>{e.stopPropagation(); closeLightbox();}); $('#lbClose').addEventListener('click',e=>{e.stopPropagation(); closeLightbox();}); $('#lbPrev').addEventListener('click',e=>{e.stopPropagation(); state.swiper?.slidePrev();}); $('#lbNext').addEventListener('click',e=>{e.stopPropagation(); state.swiper?.slideNext();}); $('#lbShare')?.addEventListener('click', e=>{ e.stopPropagation(); shareCurrentWork(); }); $('#lbCopy')?.addEventListener('click', e=>{ e.stopPropagation(); copyCurrentLink(); }); initSwiperWhenReady(index); updateLightbox(index,true); window.addEventListener('keydown',onLightboxKey);
}
...
function buildWantUrl(item, url = currentWorkUrl(item)) {
  const text = `Здравствуйте! Хочу торт по примеру: ${item?.title || 'работа из галереи'} (${url}). Мои пожелания: `;
  return `https://wa.me/79119038886?text=${encodeURIComponent(text)}`;
}
