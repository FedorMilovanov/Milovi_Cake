...
function openLightbox(index){
  ...
  $('#lbRoot').addEventListener('click',e=>{ if(e.target.id==='lbRoot') closeLightbox(); }); $('.lb-backdrop')?.addEventListener('click',e=>{e.stopPropagation(); closeLightbox();}); $('#lbClose').addEventListener('click',e=>{e.stopPropagation(); closeLightbox();}); $('#lbPrev').addEventListener('click',e=>{e.stopPropagation(); state.swiper?.slidePrev();}); $('#lbNext').addEventListener('click',e=>{e.stopPropagation(); state.swiper?.slideNext();}); $('#lbShare')?.addEventListener('click', e=>{ e.stopPropagation(); shareCurrentWork(); }); $('#lbCopy')?.addEventListener('click', e=>{ e.stopPropagation(); copyCurrentLink(); }); $$('#lbRoot [data-copy-msg]').forEach(a=>a.addEventListener('click',()=>copyWishText(a.dataset.msg||''))); initSwiperWhenReady(index); updateLightbox(index,true); window.addEventListener('keydown',onLightboxKey);
}
function initSwiperWhenReady(index){ let tries=0; const init=()=>{ if(!window.Swiper){ if(++tries < 60) setTimeout(init,50); return; } const mobile=window.innerWidth<768; state.swiper=new Swiper('#lbSwiper',{effect:'coverflow',grabCursor:true,centeredSlides:true,slidesPerView:'auto',initialSlide:index,speed:600,keyboard:{enabled:true},observer:true,observeParents:true,coverflowEffect:{rotate:mobile?18:28,stretch:mobile?-20:0,depth:mobile?150:280,modifier:1.3,slideShadows:false},on:{slideChange(){updateLightbox(this.activeIndex);},init(){this.slideTo(index,0,false); updateLightbox(index,true); requestAnimationFrame(()=>this.update());}}}); }; requestAnimationFrame(init); }
function updateLightbox(index, immediate=false){ state.lbIndex=index; const item=state.visible[index]; if(!item)return; $('#lbTitle').textContent=item.title; $('#lbDesc').textContent=item.desc; $('#lbCounter').textContent=`${String(index+1).padStart(2,'0')} / ${String(state.visible.length).padStart(2,'0')}`; $('#lbPrev').style.display=index>0?'flex':'none'; $('#lbNext').style.display=index<state.visible.length-1?'flex':'none'; $$('#lbThumbs .lb-thumb').forEach((t,i)=>t.classList.toggle('active',i===index)); $('#lbThumbs').children[index]?.scrollIntoView({behavior:immediate?'auto':'smooth',block:'nearest',inline:'center'}); const bg=$('#lbBackdropBg'); const setBg=()=>{bg.style.backgroundImage=`url("${item.src}")`; bg.classList.remove('is-switching');}; if(immediate)setBg(); else{bg.classList.add('is-switching'); clearTimeout(state.bgTimer); state.bgTimer=setTimeout(setBg,90);} $$('#lbWrapper video').forEach((v,i)=>{v.controls=i===index; if(i===index)v.play().catch(()=>{}); else v.pause();}); const url = `${location.origin}${location.pathname}#${encodeURIComponent(item.id)}`; $('#lbWantWa')?.setAttribute('href', buildContactUrl('wa', item, url)); $('#lbWantTg')?.setAttribute('href', buildContactUrl('tg', item, url)); $('#lbWantMax')?.setAttribute('href', buildContactUrl('max', item, url)); $$('#lbRoot [data-copy-msg]').forEach(a=>{ a.dataset.msg = buildWishText(item, url); }); history.replaceState(null,'',`#${item.id}`); }
...
function buildContactUrl(channel, item, url = currentWorkUrl(item)) {
  ...
  if (channel === 'tg') return `https://t.me/MiloviCake`;
...
async function copyCurrentLink() {
  ...
  try { await navigator.clipboard.writeText(url); const b=$('#lbCopy'); if(b){ const t=b.textContent; b.textContent='Ссылка скопирована'; setTimeout(()=>b.textContent=t,1400); } }
